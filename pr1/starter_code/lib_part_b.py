...
def createStateVector(pos, dir, goal, key, door, keyPickedUp): #here the custom state vector format is defined
    ...
    state[10:12] = np.zeros(2,dtype=np.int16) #we assume that initially we have not attempted to unlock either doors
    return state

def createCurrentStateVector(currentPos, currentDir, goalPos, keyPos, doorState, currentKeyPickedUp, door1UnlockAttempted, door2UnlockAttempted): #here we create a custom current state vector based on information from the environment at a certain time step
    state = np.zeros((1,13), dtype=np.int16).flatten()
...
def getNextPossibleStates(currentState, randomMap, DOOR_1_POS): #given a state vector node, what are the possible next states?
    ...
    if (np.array_equal(frontPos,goalPos)): #if there is a potential for the cell in front of us to be a goal
        frontObject = GOAL #consider it a goal if it has been randomly placed there
    elif (np.array_equal(frontPos,keyPos) and not keyPickedUp): #if there is a potential for the cell in front of us to be a key and we have not picked up a key
        ...
        if (np.array_equal(frontPos,DOOR_1_POS)): #if the door is at position 1
            if (doorState[0] or door1UnlockAttempt): #if the door at position 1 is randomly unlocked already, or if we had unlocked it in the past
                frontObject = NONE #consider the door open and to be walk throughable
            else:
                frontObject = DOOR #else, consider the door to be closed
        else:
            if (doorState[1] or door2UnlockAttempt): #if the door at position 1 is randomly unlocked already, or if we had unlocked it in the past
                frontObject = NONE #consider the door open and to be walk throughable
            else:
                frontObject = DOOR #else, consider the door to be closed
    ...
    if (frontObject == NONE or frontObject == GOAL): #if there is nothing ahead of us or if the goal is ahead of us
        possibleState = currentState.copy()
        possibleState[0:2] = possibleState[0:2] + frontDirection #we can move forward
        possibleStates[0,:] = possibleState
    elif (frontObject == KEY): #if there is a key in front of us
        possibleState = currentState.copy()
        possibleState[10] = 1 #pick it up
        possibleStates[3,:] = possibleState
    elif (frontObject == DOOR and keyPickedUp): #if there is a door in front of us and we have a key in hand
        if (np.array_equal(frontPos,DOOR_1_POS)): #if this door is door number 1
            possibleState = currentState.copy()
            possibleState[11] = 1 #attempt at unlocking door number 1
            possibleStates[4,:] = possibleState
        else: #then this is door number 2 we are facing
            possibleState = currentState.copy()
            possibleState[12] = 1 #attempt at unlocking door number 2
            possibleStates[4,:] = possibleState
...
def calculateSingleOptimalPolicy(timeHorizon, goalLocations, keyLocations, doorLocations, dimension, initialPos, initialDir):
    ...
    print("CALCULATING SINGLE OPTIMAL POLICY NOW, SHOULD TAKE UNDER 20 SECONDS")
