...
def value_iteration(mdp, num_iter):
  ...
  for k in range(num_iter):
    print(ntrm_sta.shape)
    print(mdp.L[ntrm_sta,:].shape)
    print(mdp.P[ntrm_sta,:,:].shape)
    print(V[k,None,None,:].shape)
    print((mdp.P[ntrm_sta,:,:] * V[k,None,None,:]).shape)
    print(np.sum(mdp.P[ntrm_sta,:,:] * V[k,None,None,:], axis=2).shape)
